...
import { OrderDomainService } from '../../../domain/mobile-app/orderDomain';
...
class OrderHandler {
    constructor(private service: OrderDomainService) { }
    ...
    list = async (req: Request, res: Response): Promise<any> => {
        try {
            ...
            const result = await this.service.list({ page, limit, type, userId, orderStatus, paymentStatus, orderCode });
...
export function OrderHandlerFun(service: OrderDomainService): OrderHandler {
    return new OrderHandler(service);
}