...
class AdminUserAuthMiddleware {
  ...
  public ValidateUser = async (
    ...
    try {
      ...
      if (userDtls) {
        const eistUser = await AdminUsers.findOne({ phoneNumber: decoded?.phone, isDelete: 0, isActive: 1 });
      
        if (!eistUser) {
          res.status(401).json({ message: "Invalid token" });
          return;
        }
        req.user = { id: decoded?.id.toString(), email: userDtls?.email, userType: 'AdminUser' };
        next();
      } else {
...
export function AdminUserAuthRegister(auth: AuthMiddlewareDomain) {
  return new AdminUserAuthMiddleware(auth);
}
